...
-- Date: 2026-03-29
-- Description: CASO 3 verbal-prevalece + Anti-inflation guard on corte 4↔6
--   Content derived from v0.4.4 (which was reverted due to deployment issues, not content issues)
-- Changes from v0.4.2:
--   M1: CASO 3 — resposta verbal prevalece em divergência extrema (com fallback para Pill)
--   M2: Anti-inflation guard — corte 4↔6 exige persistência sob adversidade
--   M3: B.6 — CASO 3 documentado como verbal-prevalece
-- Predecessor: v0.4.2
...
-- ============================================================
-- Step 1: Insert 16 v0.4.5 scoring block prompts
-- ============================================================

-- --- Block L11 ---
-- v0.4.5 — scoring_block_L1.1
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L1.1'
  AND version != 'v0.4.5'
  AND active = true;
...
# Changelog v0.4.1 → v0.4.2 (4 camadas anti-null):
#   C1: Regra Cardinal — INDETERMINADO proibido quando resposta presente
#   C2: Pill fallback reforçado — CASO 5 nunca retorna null se pill tem dado
#   C3: Re-ask flagging — flags.reask_recomendado quando scoring incompleto
#   C4: Inferência dimensional — implementada no questionnaire-engine
# Changelog v0.4.2 → v0.4.5:
#   M1: CASO 3 — resposta verbal prevalece em divergência extrema (com fallback para Pill)
#   M2: Anti-inflation guard — corte 4↔6 exige persistência sob adversidade
#   M3: B.6 — CASO 3 documentado como verbal-prevalece
...
## REGRA CARDINAL — PROIBIÇÃO DE INDETERMINADO COM RESPOSTA PRESENTE

**Esta regra tem precedência sobre qualquer instrução de corte nas seções seguintes.**

SE `principal_resposta` contém conteúdo narrativo (≥2 palavras que formam sentido):
→ Você DEVE decidir SIM ou NÃO para TODOS os 3 cortes (2_4, 4_6, 6_8). INDETERMINADO é **PROIBIDO**.
→ Ausência de evidência do construto na resposta = **NÃO** com GCC "baixo".
→ O respondente teve a oportunidade de demonstrar o construto e não demonstrou. Isso é dado, não lacuna.
→ Exemplo: se a resposta fala de adaptação social mas não menciona o construto deste bloco → todos os cortes = NÃO, GCC "baixo", evidência = "resposta não contém evidência do construto medido".

INDETERMINADO só é permitido quando:
(a) `principal_resposta` é null ou string vazia
(b) `protecao_etica = true`
(c) resposta é literalmente ininteligível (caracteres sem sentido, idioma incompreensível)

**Qualquer outro uso de INDETERMINADO é ERRO DE SCORING que cascata para il_canonico = null.**
...
REGRA DE RIGOR NO CORTE 4↔6: A presença do construto NÃO é suficiente para SIM. O corte 4↔6 exige que o construto PERSISTA sob condições adversas (pressão, ausência de estrutura externa, custo real). Se a resposta demonstra que o construto existe mas COLAPSA quando testado — isso é NÃO no corte 4↔6, com GCC "alto". Presença sem sustentação = Faixa B, não C.
...
→ SE `il_questionario = null` → `confianca = "baixa"`, `flags.baixa_confianca = true`, `flags.reask_recomendado = true`
...
→ **A resposta verbal prevalece sobre a Pill em divergência extrema.**
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L1_1 = true`
→ `flags.heterogeneidade_contextual_L1_1 = true`
→ JUSTIFICATIVA OBRIGATÓRIA: explicar por que Pill e Questionário divergem em ≥2 faixas.
→ SE `il_questionario = null` ou `faixa_questionario = "indeterminada"`: `il_canonico = IL_pill` (fallback — Pill quando Questionário não produz valor). SE ambos null → `il_canonico = null`, `faixa_final = "indeterminada"`, `confianca = "baixa"`, `flags.baixa_confianca = true`.
...
→ **Pill fallback:** SE `il_questionario = null` E `il_sinais` não-vazio → `il_canonico = IL_pill`, `flags.pill_fallback_ativado = true`. Nunca retornar null quando Pill tem dado.
→ `confianca = "baixa"`
...
O ceiling aplica-se ao **il_questionario quando ele é a fonte determinante**. Em CASO 1/2, a Pill prevalece com peso pleno. Em CASO 3, a resposta verbal prevalece (divergência extrema) — il_canonico não é afetado pelo ceiling do Questionário.
...
```json
{
  ...
  "flags": {
    ...
    "pill_fallback_ativado": false,
    "reask_recomendado": false
  }
}
```
...
REGRA DE COBERTURA (Camadas 1+2):
  il_canonico = null é aceitável SOMENTE quando:
  (a) protecao_etica = true E pills sem cobertura, OU
  (b) principal_resposta = null E pills sem cobertura (n_pills_com_cobertura = 0 ou il_sinais vazio).
  Em qualquer outro cenário, ao menos uma fonte DEVE produzir valor.

SE flags.reask_recomendado = true
  → scoring incompleto, bloco deve ser re-apresentado ao respondente
...
-- --- Block L12 ---
-- v0.4.5 — scoring_block_L1.2
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L1.2'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L1_2 = true`
...
-- --- Block L13 ---
-- v0.4.5 — scoring_block_L1.3
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L1.3'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L1_3 = true`
...
-- --- Block L14 ---
-- v0.4.5 — scoring_block_L1.4
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L1.4'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L1_4 = true`
...
-- --- Block L21 ---
-- v0.4.5 — scoring_block_L2.1
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L2.1'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L2_1 = true`
...
-- --- Block L22 ---
-- v0.4.5 — scoring_block_L2.2
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L2.2'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L2_2 = true`
...
-- --- Block L23 ---
-- v0.4.5 — scoring_block_L2.3
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L2.3'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L2_3 = true`
...
O ceiling aplica-se ao **il_questionario quando ele é a fonte determinante**. Em CASO 1/2, a Pill prevalece com peso pleno. Em CASO 3, a resposta verbal prevalece (divergência extrema).
...
-- --- Block L24 ---
-- v0.4.5 — scoring_block_L2.4
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L2.4'
  AND version != 'v0.4.5'
  AND active = true;
...
SE (Descoberta inscorable OU ausente)
  ...
  SE faixa_preliminar ≠ "indeterminada" E confianca_fase_a = "alta":
    → Usar faixa_preliminar como fallback (sinais contextuais)
    → il_canonico = valor canônico da faixa_preliminar (A=1.5, B=4.0, C=6.0, D=7.5)
    → faixa_final = faixa_preliminar
    → confianca = "baixa"
    → flags.pill_fallback_ativado = true
  ...
  SENÃO:
    ...
    → flags.reask_recomendado = true
```

L2.4 pode produzir `il_canonico = null` quando nenhuma fonte fornece informação. O sistema sinaliza L2.4 como dado pendente para re-ask.
...
-- --- Block L31 ---
-- v0.4.5 — scoring_block_L3.1
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L3.1'
  AND version != 'v0.4.5'
  AND active = true;
...
REGRA DE RIGOR NO CORTE 4↔6: A presença do construto NÃO é suficiente para SIM. O corte 4↔6 exige que o construto PERSISTA sob tensão relacional real (conflito, ambiguidade, pressão do outro). Se a resposta demonstra que o construto existe mas COLAPSA quando testado — isso é NÃO no corte 4↔6, com GCC "alto". Presença sem sustentação = Faixa B, não C.
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L3_1 = true`
...
-- --- Block L32 ---
-- v0.4.5 — scoring_block_L3.2
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L3.2'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L3_2 = true`
...
-- --- Block L33 ---
-- v0.4.5 — scoring_block_L3.3
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L3.3'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L3_3 = true`
...
-- --- Block L34 ---
-- v0.4.5 — scoring_block_L3.4
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L3.4'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L3_4 = true`
...
-- --- Block L41 ---
-- v0.4.5 — scoring_block_L4.1
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L4.1'
  AND version != 'v0.4.5'
  AND active = true;
...
REGRA DE RIGOR NO CORTE 4↔6: A presença do construto NÃO é suficiente para SIM. O corte 4↔6 exige que o construto PERSISTA sob incerteza sistêmica real (pressão temporal, múltiplas variáveis, ausência de referência). Se a resposta demonstra que o construto existe mas COLAPSA quando testado — isso é NÃO no corte 4↔6, com GCC "alto". Presença sem sustentação = Faixa B, não C.
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L4_1 = true`
...
-- --- Block L42 ---
-- v0.4.5 — scoring_block_L4.2
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L4.2'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L4_2 = true`
...
-- --- Block L43 ---
-- v0.4.5 — scoring_block_L4.3
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L4.3'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L4_3 = true`
...
-- --- Block L44 ---
-- v0.4.5 — scoring_block_L4.4
-- Deploy individual block

UPDATE prompt_versions
SET active = false, deprecated_at = NOW()
WHERE component = 'scoring_block_L4.4'
  AND version != 'v0.4.5'
  AND active = true;
...
→ `il_canonico = il_questionario` (valor determinado em A.6)
→ `confianca = "baixa"`
→ `flags.divergencia_extrema_L4_4 = true`
...
-- ============================================================
-- Step 2: Verification query (run after migration)
-- ============================================================
-- SELECT component, version, active, LENGTH(prompt_text) as chars
-- FROM prompt_versions
-- WHERE component LIKE 'scoring_block_L%'
--   AND version IN ('v0.4.2', 'v0.4.5')
-- ORDER BY component, version;
-- Expected: 16 rows v0.4.2 (active=false) + 16 rows v0.4.5 (active=true)
