-- C1.3: Adicionar cycle_state à tabela cycles
-- cycle_state: determinado pelo IPE, consumido como input externo pelo engine
-- Valores válidos: 'S0' | 'S1' | 'S2'
-- Default 'S0': ciclos históricos (pré-B1.3) operam em modo aberto
ALTER TABLE cycles ADD COLUMN cycle_state VARCHAR NOT NULL DEFAULT 'S0';
ALTER TABLE cycles ADD CONSTRAINT cycles_cycle_state_check
  CHECK (cycle_state IN ('S0', 'S1', 'S2'));

-- MD-3: Adicionar llm_prompt_hash para auditabilidade do system prompt
-- Valor persistido via UPDATE na PHASE 9.1 (pós-commit, pós-LLM)
-- Não entra no RPC atômico — hash é calculado durante executeLlmLanguage
ALTER TABLE cycles ADD COLUMN llm_prompt_hash VARCHAR;

-- Recriar lucid_persist_cycle com p_cycle_state
-- R2.4: p_llm_prompt_hash removido — valor real vem de UPDATE pós-commit
...
-- Drop overload anterior sem cycle_state para evitar ambiguidade
-- R1.3: O DROP abaixo usa IF EXISTS — falha silenciosa se assinatura diferir.
-- Após aplicar esta migration, verificar no Supabase SQL Editor:
--   SELECT oid::regprocedure FROM pg_proc WHERE proname = 'lucid_persist_cycle';
-- Deve retornar exatamente UMA linha. Se retornar duas, há overload ambíguo
-- e o cliente JS pode chamar a função errada.
DROP FUNCTION IF EXISTS public.lucid_persist_cycle(
  ...
  numeric, character varying, jsonb, character varying, text
);
