-- Change 1: Add llm_response column
ALTER TABLE cycles ADD COLUMN llm_response TEXT;

-- Change 2: Update the RPC that includes hago_state to also accept llm_response
...
-- Drop the old overload without hago_state to avoid ambiguity
DROP FUNCTION IF EXISTS public.lucid_persist_cycle(
  ...
  numeric, character varying, jsonb
);